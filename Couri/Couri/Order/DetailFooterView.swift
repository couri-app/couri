...
class DetailFooterView: UICollectionReusableView {
    ...
    @objc func decreaseCount() {
        ...
        if itemCount > 2 {
            itemCount -= 1
            countLabel.text = String(itemCount)
            menuDetailVC.updatedOrderPrice = orderPrice*Double(itemCount)
        } else {
            itemCount = 1
            menuDetailVC.updatedOrderPrice = orderPrice
            menuDetailVC.priceLabelSimple.text = "$\(String(format: "%.2f", (updatedOrderPrice)))"
            countLabel.text = String(itemCount)
